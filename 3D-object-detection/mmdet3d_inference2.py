...
def ensure_bbox7(bbox_tensor):
    """Ensure the bbox tensor has at least 7 elements and return the first 7."""
    arr = np.asarray(bbox_tensor, dtype=float).reshape(-1)
    if arr.shape[0] < 7:
        raise ValueError("Bounding box tensor must have at least 7 values")
    return arr[:7]
...
def get_3d_box_corners(bbox_tensor):
    ...
    """
    bbox7 = ensure_bbox7(bbox_tensor)
    center = bbox7[:3]
    extent = bbox7[3:6] # l, w, h
    yaw = bbox7[6]
...
def get_bbox_top_center(bbox_tensor):
    ...
    """
    x, y, z, dx, dy, dz, heading = ensure_bbox7(bbox_tensor)
