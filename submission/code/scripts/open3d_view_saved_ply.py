...
def main():
    ...
    parser.add_argument("--no-show", action="store_true",
                        help="Skip interactive window (useful when only saving screenshots)")
    ...
    window_visible = not args.no_show
    vis = o3d.visualization.Visualizer()
    vis.create_window(window_name=f"PLY Viewer: {base}",
                      width=args.width,
                      height=args.height,
    ...
    if args.save_path:
        vis.capture_screen_image(args.save_path, do_render=True)
        print(f"[INFO] Saved screenshot to {args.save_path}")
